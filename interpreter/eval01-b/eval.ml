...
(* eval01 : interpreter for algebraic effects and handlers *)
...
let rec f1 e xs vs c t m = match e with
  ...
  | TryWithD (e0, x, k, e1) ->
    let rec h v c1 t1 c0 t0 m0 = 
      f1 e1 (k :: x :: xs)
        (VFun (fun v2 c2 t2 m2 -> c1 v2 t1 (MCons ((c2, t2, h), m2)))
         :: v :: vs)
        c0 t0 m0 in
    f1 e0 xs vs idc TNil (MCons ((c, t, h), m))
  | TryWithS (e0, x, k, e1) ->
    let h v c1 t1 c0 t0 m0 = 
      f1 e1 (k :: x :: xs)
        (VFun (fun v2 c2 t2 m2 -> c1 v2 (apnd t1 (cons c2 t2)) m2)
         :: v :: vs)
        c0 t0 m0 in
    f1 e0 xs vs idc TNil (MCons ((c, t, h), m))
  | Call e ->
    f1 e xs vs (fun v t' m' ->
        begin match m' with
            MCons ((c0, t0, h), m0) -> h v c t' c0 t0 m0
