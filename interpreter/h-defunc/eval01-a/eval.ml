...
let rec f1 e xs vs c t m = match e with
  ...
  | TryWith (e0, x, k, e1) ->
    f1 e0 xs vs idc TNil (MCons ((c, t, (x, k, e1, xs, vs)), m))
  | CallD e ->
    f1 e xs vs (fun v t m ->
        begin match m with
            MCons ((c0, t0, (x, k, e', xs', vs')), m0) ->
            let vc = VFun (fun v' c' t' m' ->
                c v' t (MCons ((c', t', (x, k, e', xs', vs')), m'))) in
            f1 e' (x :: k :: xs') (v :: vc :: vs') c0 t0 m0
  ...
  | CallS e ->
    f1 e xs vs (fun v t m ->
        begin match m with
            MCons ((c0, t0, (x, k, e', xs', vs')), m0) -> 
            let vc = VFun (fun v' c' t' m' -> c v' (apnd t (cons c' t')) m') in
            f1 e' (x :: k :: xs') (v :: vc :: vs') c0 t0 m0
